...
int nwc_udp_client(struct nwc_args *na)
{
    ...
    destaddr = (struct sockaddr*)&serveraddr;
    destaddrlen = sizeof(serveraddr);
    ...
    printf("nwc client. slot_send_count(%d) infinit(%d) loop_count(%d) echo_mode(%d) sleep_time(%d) isconnect(%d)\n",
            slot_send_count,
            infinit,
            loop_count,
            echo_mode,
            sleep_time, na->is_udp_connect);

    while(1){
        rc = sendto(sock_fd, send_msg, send_msg_len, 0, destaddr, destaddrlen); 
        if (rc == -1){
        ...
        if (echo_mode != 2){
            rc = recvfrom(sock_fd, recv_buffer, 7, 0, destaddr, &destaddrlen);
            if (rc == 0){
